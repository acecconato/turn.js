...
let divAtt = function (top, left, zIndex, overf) {
  return {
    css: {
      position: "absolute",
      top: top,
...
let bezier = function (p1, p2, p3, p4, t) {
  let mum1 = 1 - t,
...
let rad = function (degrees) {
  return (degrees / 180) * PI;
};

// Converts an angle from radians to degrees

let deg = function (radians) {
...
let translate = function (x, y, use3d) {
  return has3d && use3d
...
let has = function (property, object) {
...
let gradient = function (obj, p0, p1, colors, numColors) {
  let j,
    cols = [];
...
let turnMethods = {
  ...
  _makeFlip: function (page) {
    ...
    if (!data.pages[page] && data.pagePlace[page] == page) {
      let single = data.display == "single",
        even = page % 2;
  ...
  range: function (page) {
    ...
    let view = turnMethods._view.call(this, page);
  ...
  _turnPage: function (page) {
    ...
    if (data.pages[current]) {
      let opts = data.pages[current].data().f.opts;
      data.tpage = next;
  ...
  update: function () {
    ...
    if (data.pageMv.length && data.pageMv[0] !== 0) {
      ...
      for (page in data.pageWrap) {
        if (!has(page, data.pageWrap)) continue;
        let pageLocation = turnMethods._setPageLoc.call(this, page);
        if (data.pages[page])
          data.pages[page]
...
let flipMethods = {
  ...
  _addPageWrapper: function () {
    ...
    if (!data.wrapper) {
      ...
      if (!data.fparent) {
        let fparent = $("<div/>", { css: { "pointer-events": "none" } }).hide();
  ...
  _fold: function (point) {
    let that = this,
      ...
      compute = function () {
        ...
        let endingPoint = flipMethods._c2.call(that, point.corner),
          far = Math.sqrt(
      ...
      transform = function (tr, c, x, a) {
        ...
        data.fpage
          .parent()
          .transform(
            rotate(a) + translate(tr.x + df.x - mv.x, tr.y + df.y - mv.y, ac),
            x
          );
  ...
  _showFoldedPage: function (c, animate) {
    let folding = flipMethods._foldingPage.call(this),
      dd = this.data(),
      data = dd.f;
  ...
  hideFoldedPage: function (animate) {
    ...
    if (animate) {
      let p4 = flipMethods._c.call(this, p1.corner),
  ...
  _eventMove: function (e) {
    ...
    if (!data.disabled) {
      ...
      if (data.corner) {
        ...
        let corner = flipMethods._cornerActivated.call(this, e[0]);
        if (corner) {
          let origin = flipMethods._c.call(
            this,
            corner.corner,
...
let cla = function (that, methods, args) {
